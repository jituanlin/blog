...
`monoid.monoidSum`和`monoid.monoidString`分别对应number类型(加法)和string类型的实现,其实现为:
```typescript
...
笔者认为"是否采用严格控制副作用"应遵循社区主流实践, 如JS社区未采用严格控制副作用的编程范式, 故不应将架构特意设计为控制副作用的编程范式,  
